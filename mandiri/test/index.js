...
describe("API ENDPOINT TESTING", () => {
  it("GET Landing Page", (done) => {
    ...
    chai
      ...
      .end((err, res) => {
        expect(err).to.be.null //  ======>    cek jika terjadi err akan di tampilkan erorr nya
        expect(res).to.have.status(200) // cek status code  harus 200
        expect(res.body).to.be.an("object") //  ======>    cek respon body harus object
        expect(res.body).to.have.property("hero") //  ======>    cek respon harus punya properti / key 'hero'
        expect(res.body.hero).to.have.all.keys(
          "travelers",
          "treasures",
          "cities"
        ) //  ======>    cekrspon harus memiliki semua nama key tersebut
        expect(res.body).to.have.property("mostPicked") //  ======>    cek respon harus ada properti / key 'mostPicked'
        expect(res.body.mostPicked).to.have.an("array") //  ======>    cek respon mostPicked harus bertipe data array
        expect(res.body).to.have.property("category") //  ======>    cek respon harus ada key 'category'
        expect(res.body.category).to.have.an("array") //  ======>    cek respon category harus bertipe data array
        expect(res.body).to.have.property("testimonial") //  ======>    cek harus ada key 'testimonial'
        expect(res.body.testimonial).to.have.an("object") //  ======>    cek respon testimonial harus berupa object
        done()
      })
  })
  ...
  it("POST Booking Page", (done) => {
    ...
    chai
      ...
      .field("idItem", dataSample.idItem) //  ======>    tes ngirim request berupa idItem dengan isi dataSample.idItem
      .field("duration", dataSample.duration) // tes request berupa dat
